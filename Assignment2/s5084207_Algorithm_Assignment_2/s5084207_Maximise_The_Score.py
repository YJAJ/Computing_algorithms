...
class ScoreMaximiser:
    ...
    #change the turn
    def toggle(self, toggle_state):
        toggled = toggle_state
        if toggle_state == "HEADS":
            toggled = "TAILS"
        if toggle_state == "TAILS":
            toggled = "HEADS"
        return toggled
    ...
    def format_content(self):
        ...
        for index in range(1, num_games+1):
            ...
            if index%3==0:
                ...
                track_set += 1
            if track_set==3:
                games_list.append(game_setting)
                game_setting = {"n_balls": 0, "n_turns": 0, "ball_list": [], "turn": ""}
                track_set = 0
        return num_game, games_list
...
if __name__=="__main__":
    try:
        ...
        print("File %s does not exist. Please check your file name and try again." % sys.argv[1])
        sys.exit()
    ...
    for each_game in range(no_game):
        ...
        with open("outputTheScore.txt", 'a') as output_file:
